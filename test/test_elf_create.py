...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Test 5: PIC binaries (Issue #274). R_ARM_BASE_PREL (25) used to be
        ...
        velf5 = os.path.join(tmpdir, "sample_pic.velf")
        res5 = subprocess.run([elf_create, sample_pic_elf, velf5], capture_output=True, text=True)
        if res5.returncode != 0:
            print("Regression (#274): vita-elf-create failed on a PIC binary:", res5.stderr)
            sys.exit(1)

        secs5 = inspect_velf_sections(velf5)
        rel5 = secs5[".sce.rel"]["data"]
        R_ARM_ABS32 = 2
        R_ARM_REL32 = 3
        got_base_rel32 = got_slot_abs32 = False
        for off in range(0, len(rel5), 12):
            w1, w2, w3 = struct.unpack_from('<III', rel5, off)
            code = (w1 >> 8) & 0xFF
